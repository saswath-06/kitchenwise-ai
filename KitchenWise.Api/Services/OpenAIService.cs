...
namespace KitchenWise.Api.Services
{
    ...
    {
        Task<List<RecipeDto>> GenerateRecipesAsync(List<string> ingredients, string? cuisine = null, string? difficulty = null);
        Task<DetailedRecipeDto?> GetRecipeDetailsAsync(string recipeName, List<string> ingredients);
        Task<string?> GenerateRecipeImageAsync(string recipeName, string recipeDescription, string cuisine);
    }
            ...
            try
            {
                ...
                if (jsonStart >= 0 && jsonEnd > jsonStart)
                {
                    ...
                    foreach (var element in jsonDocument.RootElement.EnumerateArray())
                    {
                        ...
                        {
                            Id = Guid.NewGuid(),
                            Name = GetStringValue(element, "name") ?? "Unknown Recipe",
                            Description = GetStringValue(element, "description") ?? "",
                            Cuisine = GetStringValue(element, "cuisine") ?? "International",
                            DifficultyLevel = GetStringValue(element, "difficultyLevel") ?? "Medium",
                            PrepTimeMinutes = GetIntValue(element, "prepTimeMinutes") ?? 15,
                            CookTimeMinutes = GetIntValue(element, "cookTimeMinutes") ?? 25,
                            Servings = GetIntValue(element, "servings") ?? 4,
                            Ingredients = GetStringArrayValue(element, "ingredients"),
                            Instructions = GetStringArrayValue(element, "instructions"),
                            CreatedAt = DateTime.UtcNow,
                            Nutrition = new NutritionDto(), // Default nutrition
        ...
        private DetailedRecipeDto? ParseDetailedRecipeFromResponse(string content, string recipeName)
        {
            try
            {
                ...
                if (jsonStart >= 0 && jsonEnd > jsonStart)
                {
                    ...
                    {
                        Id = Guid.NewGuid(),
                        Name = GetStringValue(element, "name") ?? recipeName,
                        Description = GetStringValue(element, "description") ?? "",
                        Cuisine = GetStringValue(element, "cuisine") ?? "International",
                        DifficultyLevel = GetStringValue(element, "difficultyLevel") ?? "Medium",
                        PrepTimeMinutes = GetIntValue(element, "prepTimeMinutes") ?? 15,
                        ...
                        CreatedAt = DateTime.UtcNow,
                        ImageUrl = null,
                        // Enhanced details for DetailedRecipeDto
                        DetailedInstructions = GetStringArrayValue(element, "instructions"), // Same as instructions for now
                        Tips = GetStringArrayValue(element, "tips"),
                        Variations = new string[0], // Will be empty for now
                        Equipment = new string[0], // Will be empty for now
                        EstimatedCost = 0 // Will be 0 for now
                    };
